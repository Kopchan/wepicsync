...
public partial class AlbumSyncPopupViewModel : ObservableObject
{
    ...
    public async Task Confirm(AlbumRemote? remoteAlbum = null, CancellationToken token = default)
    {
        if (remoteAlbum == null)
        {
            Console.WriteLine("Where the null the exception 0");
            string finalName;
            if (AlbumNameNew != "")
            {
                Console.WriteLine("Where the null the exception 0i");
                ...
                Console.WriteLine("Where the null the exception 0ii");
            }
            else
            {
                Console.WriteLine("Where the null the exception 0e");
                finalName = AlbumNameDefault;
            }

            Console.WriteLine("Where the null the exception 1");
            (var res, var body) = await FetchAsync<AlbumRemote>(
                HttpMethod.Post, URLs.AlbumsOwn,
                f => IsBusy = f, e => Error = e,
                new AlbumCreateRequest(finalName), 
                serialize: true,
                cancellationToken: token
            );
            Console.WriteLine("Where the null the exception 2");

            if (body == null) return;

            remoteAlbum = body;
            Console.WriteLine("Where the null the exception 3done");
        }
