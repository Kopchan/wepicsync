    ...
    public static async Task<bool> TrySaveAndNavigate(
        ...
        try
        {
            ...
            (var res, var body) = await FetchAsync<ServerSettingsResponse>(
                HttpMethod.Get,
                new Uri(url.ToString().TrimEnd('/') + "/api/setups"), 
                setIsFetch,
                mes => error = mes,
            ...
            if (!res.IsSuccessStatusCode)
                throw new Exception(error ?? $"Пришёл плохой код ({(int)res.StatusCode} {res.ReasonPhrase}). Возможно сервер не является экземпляром WepicSync?");

            if (error != null)
                throw new Exception(error);

            if (body == null)
                throw new Exception($"Пришёл не тот ответ, возможно сервер не является экземпляром WepicSync");
