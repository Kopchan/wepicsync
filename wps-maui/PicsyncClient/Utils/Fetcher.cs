...
public static class Fetcher
{
    ...
    public static async Task<HttpResponseMessage?> FetchAsync(
        ...
        else if (path is string urlEnd)
        {
            try
            {
                // Удаляем конечный слэш в URLs.API_URL, если он есть
                var baseUrl = URLs.API_URL.ToString().TrimEnd('/');

                // Удаляем начальный слэш в urlEnd, если он есть
                urlEnd = urlEnd.TrimStart('/');

                fullUrl = new Uri($"{baseUrl}/{urlEnd}");
                Console.WriteLine("url parts: " + baseUrl + " and " + urlEnd);
            }
            catch (Exception ex)
            {
    ...
    public static async Task<(HttpResponseMessage?, T?)> FetchAsync<T>(
        ...
        {
            // TODO: удалить дебаг ответа
            var responseJson = "не удалось преобразовать в строку";
            try
            {
